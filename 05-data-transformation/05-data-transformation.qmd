...
## We drew the owl, now let's dive into the steps you need to get this result on your own with any dataset

![](img/draw-the-owl.jpg)
...
- What if I only need to analyze a subset of the columns or rows?
- How can I remove missing values (NAs)?

## We almost always need to transform our data to get what we need out of it

![](img/transform-functions.png)

## `dplyr` functions for data transformation...

... based on the concept of functions as verbs that manipulate data frames
...
- A data frame is always returned
...
- Start with the `hotels` data frame and pass it to the `select()` function
- Select the variables `hotel` and `lead_time`
- Then arrange the data frame by `lead_time` in descending order
:::
::: {.column width = "50%"}
```{r}
#| code-line-numbers: "3"
...
# You might see `%>%`. This is also a pipe (from the `magrittr` package). R now has it's own build-in pipe operator `|>` so that's what most people use now.
